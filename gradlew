...
# Use the maximum available options if this is total memory available environment variable
if [ -n "$MAX_APP_HOME" ]; then
    APP_HOME="$MAX_APP_HOME"
fi
...
if [ "$cygwin" = "false" -a "$darwin" = "false" -a "$nonstop" = "false" ] ; then
    ...
    if [ $? -eq 0 ] ; then
        if [ "$MAX_CAN_ONLY_BE_USED_BY_ROOT" = "true" ] ; then
            if [ "$UID" -eq 0 ] ; then
